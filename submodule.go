...
func (repo *Repository) ForeachSubmodule(cbk SubmoduleCbk) error {
	return makeError(C._go_git_visit_submodule(repo.ptr, unsafe.Pointer(&cbk)))
}
...
func (sub *Submodule) FinalizeAdd() error {
	return makeError(C.git_submodule_add_finalize(sub.ptr))
}

func (sub *Submodule) AddToIndex(write_index bool) error {
	return makeError(C.git_submodule_add_to_index(sub.ptr, cbool(write_index)))
}

func (sub *Submodule) Save() error {
	return makeError(C.git_submodule_save(sub.ptr))
}
...
func (sub *Submodule) SetUrl(url string) error {
	...
	return makeError(C.git_submodule_set_url(sub.ptr, curl))
}
...
func (sub *Submodule) SetFetchRecurseSubmodules(v bool) error {
	return makeError(C.git_submodule_set_fetch_recurse_submodules(sub.ptr, cbool(v)))
}

func (sub *Submodule) Init(overwrite bool) error {
	return makeError(C.git_submodule_init(sub.ptr, cbool(overwrite)))
}

func (sub *Submodule) Sync() error {
	return makeError(C.git_submodule_sync(sub.ptr))
}
...
func (sub *Submodule) Reload() error {
	return makeError(C.git_submodule_reload(sub.ptr))
}

func (repo *Repository) ReloadAllSubmodules() error {
	return makeError(C.git_submodule_reload_all(repo.ptr))
}
